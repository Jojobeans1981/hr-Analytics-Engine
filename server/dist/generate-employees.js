...
function generateRandomEmployee(index, existingEmails) {
...
async function generateEmployees() {
    try {
        ...
        // Get existing emails
        const existingEmails = new Set(await db.collection('employees')
        ...
        if (error.code === 11000) {
            ...
            console.log(`📈 Total employees in database: ${await mongoose_1.default.connection.db.collection('employees').countDocuments()}`);
        }
        else {
